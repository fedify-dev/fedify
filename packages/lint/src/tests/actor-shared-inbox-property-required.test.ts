...
const tests = createRequiredListenerRuleTests("sharedInbox", config);
test(
...
test(
  `${ruleName}: ✅ Edge - if else if else with sharedInbox in all branches`,
  edgeCases["if else if else with property in all branches"],
);
test(
  `${ruleName}: ❌ Edge - if else if else missing sharedInbox in else if`,
  edgeCases["if else if else missing property in else if"],
);
test(
  `${ruleName}: ✅ Edge - if else if with final return sharedInbox in all paths`,
  edgeCases["if else if with final return property in all paths"],
);
test(
  `${ruleName}: ❌ Edge - if else if with final return missing sharedInbox in final return`,
