...
const tests = createRequiredDispatcherRuleTests("outbox", config);
test(
...
test(
  `${ruleName}: ✅ Edge - if else if else with outbox in all branches`,
  edgeCases["if else if else with property in all branches"],
);
test(
  `${ruleName}: ❌ Edge - if else if else missing outbox in else if`,
  edgeCases["if else if else missing property in else if"],
);
test(
  `${ruleName}: ✅ Edge - if else if with final return outbox in all paths`,
  edgeCases["if else if with final return property in all paths"],
);
test(
  `${ruleName}: ❌ Edge - if else if with final return missing outbox in final return`,
