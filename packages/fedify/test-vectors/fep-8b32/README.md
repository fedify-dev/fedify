...
The fixture records the final secured compound document, the exact secured
...
and expected verification outcomes.  JSON member order and source formatting
are not significant; the recorded JSON values and their JCS canonicalizations
are.
...
credentials.  Regenerate the fixture from the repository root with:
