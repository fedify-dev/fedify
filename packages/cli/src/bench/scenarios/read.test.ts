...
test("runReadLoad - limits resolved read URL gate concurrency", async () => {
  const scenario = normalizeSuite({
    version: 1,
    target: "http://target.test/",
    scenarios: [{
...
test("runReadLoad - waits for in-flight read URL gates after a failure", async () => {
  const scenario = normalizeSuite({
    version: 1,
    target: "http://target.test/",
    scenarios: [{
  ...
  await assert.rejects(
    async () =>
      await runReadLoad({
        ...
        assertReadDestinationAllowed: async () => {
          gateCalls++;
          if (gateCalls === 1) throw new Error("first gate failed");
          await new Promise((resolve) => setTimeout(resolve, 20));
          delayedGatesSettled++;
          throw new Error("later gate failed");
        },
      }, {
        urls,
        authenticated: false,
  ...
  assert.ok(delayedGatesSettled > 0);
  assert.ok(gateCalls < urls.length);
});
