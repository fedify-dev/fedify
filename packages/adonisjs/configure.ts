...
/**
 * The range the Fedify runtime packages are installed with.
 *
 * Derived from this package's own version so a release cannot leave it behind:
 * every package in this monorepo shares one version, and the `@fedify/fedify`
 * peer dependency is declared as `workspace:^`, which publishes as this range.
 *
 * Installing unpinned would pull the next major once it is out, which the peer
 * range rejects: a peer warning, and two copies of the vocabulary, which the
 * `instanceof` comparisons noted below cannot survive.
 */
const FEDIFY_RANGE = `^${packageJson.version}`;

/**
 * Wires `@fedify/adonisjs` into an application.
 *
 * Run by `node ace configure @fedify/adonisjs` (and therefore by
 * `node ace add @fedify/adonisjs`); see the module documentation for the
 * steps it performs.
 */
export async function configure(command: ConfigureCommand): Promise<void> {
  ...
  await codemods.installPackages([
    { name: `@fedify/fedify@${FEDIFY_RANGE}`, isDevDependency: false },
    { name: `@fedify/vocab@${FEDIFY_RANGE}`, isDevDependency: false },
  ]);
  ...
  await codemods.updateRcFile((rcFile: RcFileEditor) => {
    rcFile.addProvider("@fedify/adonisjs/fedify_provider");
   ...
   *
   * It also goes *first* in that stack, so that Fedify negotiates on the
   * client's real `Accept` header.  The API starter kit's
   * `force_json_response_middleware`, for example, rewrites every `Accept` to
   * `application/json`, which would make Fedify serve the ActivityPub document
   * to a browser asking for HTML and skip the HTML fall-through.
   */
  await codemods.registerMiddleware("server", [{
    path: "@fedify/adonisjs/fedify_middleware",
    position: "before",
  }]);
